...
16. 单主人账号登录：首次设置需要服务器口令，密码使用 `scrypt` 哈希保存；所有页面、API、截图和推送入口均要求有效会话，可选择在当前设备记住登录 30 天。
...
首次运行前还需要在 `.env.local` 配置至少 20 位的 `AUTH_SETUP_TOKEN`。打开 `/setup`，使用该口令创建唯一的主人账号；建号完成后可以从服务器环境变量中移除该口令。日常密码可在左侧栏底部的账号入口中修改，修改后其他设备会自动退出。
...
- 主人账号、密码哈希和服务端会话保存在当前环境的 SQLite 中；浏览器 Cookie 为 HttpOnly、SameSite=Lax，HTTPS 部署时自动启用 Secure；
