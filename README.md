...
16. 多用户账号与注册：首次设置需要服务器口令，之后可从登录页注册独立账号；每个账号的事项、话题、笔记、截图、AI 上下文和提醒完全隔离，可选择在当前设备记住登录 30 天。
...
首次运行前还需要在 `.env.local` 配置至少 20 位的 `AUTH_SETUP_TOKEN`。打开 `/setup`，使用该口令创建初始账号；之后其他用户可以从登录页自助注册。该口令也用于密码找回，不需要找回能力时可以从服务器环境变量中移除。日常密码可在左侧栏底部的账号入口中修改，修改后该账号的其他设备会自动退出。
